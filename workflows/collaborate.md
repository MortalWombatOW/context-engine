...
2.  **Context Synthesis**
    *   Update `{{ docs.log }}` with the latest changes.
    *   Identify the "Active Working Set": Which files are currently open or being modified?
    *   Identify "Invisible Context": What did you learn that isn't written down yet? Add this to `{{ docs.rules }}` if permanent, or the Handoff Note if transient.
...
**Status**: Implementation complete, but tests failing on edge cases.
**Active Files**: `src/systems/navigation.rs`, `src/components/companion.rs`.
**Next Steps**:
1. Debug the edge case logic in `navigation.rs`.
2. Run `{{ commands.test }}` to verify.
**Warning**: Do not increase the recursion depth, it causes stack overflows.
```
